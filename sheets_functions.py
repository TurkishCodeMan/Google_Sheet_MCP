...
CREDENTIALS_FILE = '/Users/huseyin/Documents/deep-learning/wp_agent/google-sheets-mcp/google_credentials.json'
...
def get_sheet_client():
    ...
    try:
        ...
        if os.environ.get('GOOGLE_CREDENTIALS'):
            ...
            credentials_json = os.environ.get('GOOGLE_CREDENTIALS') # çevre değişkeninden okunuyor
...
def open_sheet(spreadsheet_name_or_key=DEFAULT_SPREADSHEET, worksheet_name=DEFAULT_WORKSHEET):
    """
    Belirtilen tabloyu ve sayfayı açar.
    
    Args:
        spreadsheet_name_or_key (str, optional): Tablo adı veya benzersiz anahtarı (varsayılan: DEFAULT_SPREADSHEET)
        worksheet_name (str, optional): Sayfa adı (varsayılan: DEFAULT_WORKSHEET)
...
def create_spreadsheet(spreadsheet_name=DEFAULT_SPREADSHEET, share_with=None):
    """
    Yeni bir Google Tablosu oluşturur.
    
    Args:
        spreadsheet_name (str, optional): Oluşturulacak tablonun adı (varsayılan: DEFAULT_SPREADSHEET)
...
def get_all_reservations(spreadsheet_name_or_key=None, worksheet_name=None):
    """
    ...
    try:
        ...
        if spreadsheet_name_or_key is None:
            spreadsheet_name_or_key = DEFAULT_SPREADSHEET
        if worksheet_name is None:
            worksheet_name = DEFAULT_WORKSHEET
            
        _, worksheet = open_sheet(spreadsheet_name_or_key, worksheet_name)
        
        # Debug: Sadece worksheet nesnesinin varlığını kontrol edelim
        print(f"DEBUG: Worksheet alındı: {worksheet.title}") 
        
        # Tüm verileri sözlük listesi olarak al
        records = worksheet.get_all_records()
        print(f"DEBUG: get_all_records sonucu: {len(records)} kayıt")
        
        # Başka bir şekilde deneyelim (fallback mechanism)
        if len(records) == 0: 
            # get_all_values sadece fallback durumunda çağrılsın
            all_values = worksheet.get_all_values()
            print(f"DEBUG: Fallback - Tablo içeriği kontrolü")
            print(f"DEBUG: Fallback - Toplam satır sayısı: {len(all_values)}")
            
            if len(all_values) > 1: # Başlık dışında veri var mı?
                print("DEBUG: Fallback - Manuel kayıt oluşturma deneniyor")
                if len(all_values) > 0:
                    print(f"DEBUG: Fallback - İlk satır (headers): {all_values[0]}")
                if len(all_values) > 1:
                    print(f"DEBUG: Fallback - İkinci satır (ilk veri): {all_values[1]}")
                
                # Manuel kayıt oluşturma mantığı burada kalır
                records = []
                ...
                print(f"DEBUG: Fallback - Manuel oluşturulan kayıt sayısı: {len(records)}")
            else:
                 print("DEBUG: Fallback - Tabloda sadece başlık var veya tablo boş.")
...
def get_reservations_by_name(customer_name, exact_match=True, room_type=None, spreadsheet_name_or_key=DEFAULT_SPREADSHEET, worksheet_name=DEFAULT_WORKSHEET):
    """
    Müşteri adına göre rezervasyonları filtreler.
    
    Args:
        customer_name (str): Filtrelenecek müşteri adı
        exact_match (bool, optional): True ise tam eşleşme arar, False ise içeren kayıtları bulur (varsayılan: True)
...
def add_reservation(reservation_data, spreadsheet_name_or_key=DEFAULT_SPREADSHEET, worksheet_name=DEFAULT_WORKSHEET):
    """
...
def update_reservation(reservation_id, updated_data, spreadsheet_name_or_key=DEFAULT_SPREADSHEET, worksheet_name=DEFAULT_WORKSHEET):
    """
...
def delete_reservation(reservation_id=None, customer_name=None, room_type=None, spreadsheet_name_or_key=DEFAULT_SPREADSHEET, worksheet_name=DEFAULT_WORKSHEET):
    """
    Bir rezervasyonu siler.
    
    Args:
        reservation_id (str, optional): Silinecek rezervasyon ID'si
        customer_name (str, optional): Silinecek müşteri adı
    ...
    Not: 
        reservation_id veya customer_name parametrelerinden en az biri verilmelidir.
        Her ikisi de verilirse, reservation_id önceliklidir.
    """
    try:
        # Parametre kontrolü
        if reservation_id is None and customer_name is None:
            logger.error("Silme işlemi için reservation_id veya customer_name belirtilmelidir")
            return False
            
        logger.info(f"Rezervasyon silme işlemi başlatıldı: ID={reservation_id}, Müşteri={customer_name}, Oda Tipi={room_type}")
        _, worksheet = open_sheet(spreadsheet_name_or_key, worksheet_name)
        ...
        if customer_name is not None:
            ...
            for i, row in enumerate(all_data[1:], start=2):  # 1 tabanlı satır indeksi, başlık satırını atla
                ...
                if customer_col < len(row):
                    ...
                    customer_match = (row_customer_name and customer_name and 
                                    row_customer_name.lower() == customer_name.lower())
            ...
            if deleted_rows:
                ...
                filter_desc = f"müşteri adı '{customer_name}'"
                if room_type:
...
def check_availability(check_in_date, check_out_date, adults=2, children=0, room_type=None):
    ...
    Args:
        ...
        adults (int, optional): Yetişkin sayısı (varsayılan: 2)
        children (int, optional): Çocuk sayısı (varsayılan: 0)
